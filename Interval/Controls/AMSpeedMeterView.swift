...
@IBDesignable public class AMSpeedMeterView: UIView {
    ...
    private func prepareValueHandLayer() {
        ...
        // Contimue to use the valueHandLayer
    ...
    private func handAnimation() {
        ...
        // Use CASpringAnimation() for the daming effect of the clock
        
        // Spring enabled animation
        let rotationWithSpringAnimation = CASpringAnimation(keyPath: "transform.rotation.z")
        rotationWithSpringAnimation.damping = 13
        rotationWithSpringAnimation.fillMode = CAMediaTimingFillMode.forwards
        
        rotationWithSpringAnimation.toValue = endAngle - startAngle
    
        rotationWithSpringAnimation.duration = 0.5 // Simply set this to 1
        rotationWithSpringAnimation.isRemovedOnCompletion = false
        startAngle = endAngle
        valueHandLayer.add(rotationWithSpringAnimation, forKey: "rotateAnimation")

       /*
         fromValue = myLayer.position.x
    ...
    // MARK: PUBLIC METHODS FOR THE UI STYLING
    
    public func incrementBorderWith(width: CGFloat) {
        meterBorderLineWidth = width
    }
    
    public func getSelectedValue() -> Int {
